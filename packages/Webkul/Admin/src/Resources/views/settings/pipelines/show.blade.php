<x-admin::layouts>
    <!-- Page Title -->
    <x-slot:title>
        @lang('admin::app.settings.pipelines.index.show.title') - {{ $pipeline->name }}
    </x-slot>
    ...
    <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-1">
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.settings.pipelines.index') }}" class="text-xs font-bold text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white">
                        ← @lang('admin::app.settings.pipelines.index.show.back-btn')
                    </a>
                </div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">@lang('admin::app.settings.pipelines.index.show.subtitle')</p>
            </div>
            ...
            <div class="flex items-center gap-2">
                @if (bouncer()->hasPermission('settings.lead.pipelines.edit'))
                    <a href="{{ route('admin.settings.pipelines.edit', $pipeline->id) }}" class="primary-button">
                        @lang('admin::app.settings.pipelines.index.show.edit-btn')
                    </a>
                @endif
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">@lang('admin::app.settings.pipelines.index.show.total-stages')</span>
                <p class="text-3xl font-extrabold text-sky-600 mt-1 dark:text-sky-400">{{ $stageStats->count() }} Stage</p>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">@lang('admin::app.settings.pipelines.index.show.total-leads')</span>
                <p class="text-3xl font-extrabold text-indigo-600 mt-1 dark:text-indigo-400">{{ $leadsCount }} Lead</p>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">@lang('admin::app.settings.pipelines.index.show.rotten-days')</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            <div class="lg:col-span-4 rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 space-y-4">
                <div class="border-b border-gray-100 pb-3 dark:border-gray-800">
                    <h2 class="text-base font-extrabold text-gray-900 dark:text-white">@lang('admin::app.settings.pipelines.index.show.info-title')</h2>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">@lang('admin::app.settings.pipelines.index.show.info-subtitle')</p>
                </div>

                <div class="space-y-3.5 text-xs">
                    <div class="flex items-center justify-between py-1.5 border-b border-gray-100 dark:border-gray-800/60">
                        <span class="text-gray-500 font-medium dark:text-gray-400">@lang('admin::app.settings.pipelines.index.show.pipeline-name')</span>
                        <span class="font-extrabold text-gray-900 dark:text-white">{{ $pipeline->name }}</span>
                    </div>

                    <div class="flex items-center justify-between py-1.5 border-b border-gray-100 dark:border-gray-800/60">
                        <span class="text-gray-500 font-medium dark:text-gray-400">@lang('admin::app.settings.pipelines.index.show.default-status')</span>
                        <span class="inline-flex items-center rounded-md px-2 py-0.5 text-[11px] font-bold {{ $pipeline->is_default ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' }}">
                            {{ $pipeline->is_default ? trans('admin::app.settings.pipelines.index.show.default-yes') : trans('admin::app.settings.pipelines.index.show.default-no') }}
                        </span>
                    </div>

                    <div class="flex items-center justify-between py-1.5 border-b border-gray-100 dark:border-gray-800/60">
                        <span class="text-gray-500 font-medium dark:text-gray-400">@lang('admin::app.settings.pipelines.index.show.rotten-days')</span>
                        <span class="font-bold text-amber-600 dark:text-amber-400">{{ $pipeline->rotten_days }} Hari</span>
                    </div>

                    <div class="flex items-center justify-between py-1.5 border-b border-gray-100 dark:border-gray-800/60">
                        <span class="text-gray-500 font-medium dark:text-gray-400">@lang('admin::app.settings.pipelines.index.show.total-value')</span>
                        <span class="font-bold text-emerald-600 dark:text-emerald-400">Rp {{ number_format($totalPipelineValue, 0, ',', '.') }}</span>
                    </div>

                    <div class="flex items-center justify-between py-1.5">
                        <span class="text-gray-500 font-medium dark:text-gray-400">@lang('admin::app.settings.pipelines.index.show.created-at')</span>
            ...
            <div class="lg:col-span-8 rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 space-y-4">
                <div class="border-b border-gray-100 pb-3 dark:border-gray-800 flex items-center justify-between">
                    <div>
                        <h2 class="text-base font-extrabold text-gray-900 dark:text-white">@lang('admin::app.settings.pipelines.index.show.stages-title')</h2>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">@lang('admin::app.settings.pipelines.index.show.stages-subtitle')</p>
                    </div>
                ...
                @if($stageStats->isEmpty())
                    <div class="py-8 text-center text-xs text-gray-400 font-medium">
                        @lang('admin::app.settings.pipelines.index.show.empty-stages')
                    </div>
                @else
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs">
                            <thead class="border-b border-gray-200 bg-gray-50 text-gray-600 uppercase font-bold dark:border-gray-800 dark:bg-gray-950 dark:text-gray-400">
                                <tr>
                                    <th class="px-3 py-2.5 w-12 text-center">@lang('admin::app.settings.pipelines.index.show.order')</th>
                                    <th class="px-3 py-2.5">@lang('admin::app.settings.pipelines.index.show.stage-name')</th>
                                    <th class="px-3 py-2.5">@lang('admin::app.settings.pipelines.index.show.stage-code')</th>
                                    <th class="px-3 py-2.5">@lang('admin::app.settings.pipelines.index.show.probability')</th>
                                    <th class="px-3 py-2.5 text-center">@lang('admin::app.settings.pipelines.index.show.leads-count')</th>
                                    <th class="px-3 py-2.5 text-right">@lang('admin::app.settings.pipelines.index.show.stage-total-value')</th>
                                </tr>
                            </thead>
